...
この文書は package 構造と authoring rules を扱う。hook の実行順序と動的 URL の判断は [実行ライフサイクル](execution-lifecycle.md) を正本とする。

## package

site plugin は plugin root の <code>site_plugins/&lt;unit&gt;/</code>、image processor は <code>image_processor_plugins/&lt;unit&gt;/</code> に置く。各 unit は manifest、metadata、author defaults、entry module を持つ。署名、catalog、install、trust の規則は [配布・信頼・運用](distribution-and-operations.md) を参照する。
