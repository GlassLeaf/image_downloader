...
| 読者 | 最初に読む文書 | 目的 |
| --- | --- | --- |
| 利用者・運用者 | [CLI 参照](cli-reference.md) と [設定参照](configuration-reference.md) | 実行、全 option/JSON、設定 layer と YAML schema |
| plugin 作者 | [plugin 開発参照](plugin-development-reference.md) | package、manifest、署名、全 hook、テスト |
...
| 既存 v3 記述の追跡 | [legacy coverage ledger](legacy-v3-coverage.md) | snapshot の各有効仕様の現行移管先 |

## 読み方

実行時の hook の順序、回数、失敗処理は [実行ライフサイクル](execution-lifecycle.md) が唯一の正本である。CLI と YAML の詳細はそれぞれ [CLI 参照](cli-reference.md) と [設定参照](configuration-reference.md)、package/hook の詳細は [plugin 開発参照](plugin-development-reference.md)、公開 class の引数と戻り値は [完全 API 参照](api-reference.md) が正本である。過去の有効記述をどこへ移したかは [legacy coverage ledger](legacy-v3-coverage.md) で追跡する。
