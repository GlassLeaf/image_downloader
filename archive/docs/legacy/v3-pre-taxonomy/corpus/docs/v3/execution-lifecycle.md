...
この文書は site plugin、image processor、transport、保存の実行順序の正本である。hook の型は [完全 API 参照](api-reference.md)、package の作り方は [plugin 開発参照](plugin-development-reference.md) を参照する。
