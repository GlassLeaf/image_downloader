...
この文書は再編前リンク向けの概要である。日常操作と機械可読出力は [CLI 参照](cli-reference.md)、layer と YAML schema は [設定参照](configuration-reference.md) が正本である。プログラムからの config API は [完全 API 参照](api-reference.md) を参照する。
